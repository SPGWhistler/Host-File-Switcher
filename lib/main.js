...
- Finish windows support
	- todo:
	- figure out system host file path automatically
	- bugs:
	- when updating preferences path, menu does not refresh
	- icon isn't always accurate
	- check marks dont appear in menu
	- notes:
	- here is system host file path: C:\\WINDOWS\\system32\\drivers\\etc\\hosts
...
storage.storage.hostFiles = (!storage.storage.hostFiles) ? {
	...
	'example host file2' : {
		'data' : '# This is another example host file.\n# You can delete it, or change it.',
		'selected' : false
	}
...
var hostFileSwitcher = {
	...
	guessSystemHostFilePath : function()
	{
		...
		{
			//Windows
			//path = io.path(io.getSystemPath('SysD')).substring(8) + '/drivers/etc/hosts';
			//@TODO
			/*
			...
			console.log(path);
			*/
			//The following should work unless they have a differently named
			//WINDOWS or system dir or whatever (which is possible).
