...
exports.main = function(options){
	...
	hostFileManager = panels.Panel({
		...
		onMessage: function(message){
			...
			hostFileManager.hide();
		},
		onShow: function(){
			this.postMessage(hostFiles);
		}
	});
	...
	*/
	...
	prefs.systemHostFilePath = (prefs.systemHostFilePath !== '') ? prefs.systemHostFilePath : this.guessSystemHostFilePath();
	...
	{
		//@TODO This fails because the file doesn't exit - because I am
...
exports.hostFileClicked = function(hostFile)
{
	...
	hostFiles[hostFile].selected = (hostFiles[hostFile].selected === true) ? false : true;
	//Create new host file, and overwrite system host file with it
	if (!io.writeFile(prefs.systemHostFilePath, this.createHostFile()))
	{
		//Can't write to the system host file
		notifications.notify({
			title: 'Host File Switcher - Problem',
 ...
 */
exports.createHostFile = function()
{
	var newData = '';
	newData += (prefs.baseHostFile !== '') ? prefs.baseHostFile + "\n" : '';
	for (var i in hostFiles)
	{
		if (hostFiles[i].selected === true)
		{
			newData += hostFiles[i].data + "\n";
		}
	}
	return newData;
};
...
exports.manageHostFilesClicked = function(){
	hostFileManager.show();
};
exports.getHostFiles = function(){
	return hostFiles;
};
