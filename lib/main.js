...
var hostFileSwitcher = {
	...
	syncHostFiles : function()
	{
		...
		//@TODO This doesn't need to be global - even if we move all this to seperate functions, it can just be a param.
		...
		if (patt.test(unescape(haystack)))
		{
			...
			storage.storage.hostFiles[title] = {
				'data' : this.tmpSystemHostFileContents,
				'selected' : true
			};
			//Write out new host file
			//@TODO This is actually different then just writting out what was originally there, in that contents
			//are not added in the same order. Not sure there is a good way around that because of how the base
			//host file gets added.
